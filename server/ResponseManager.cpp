...
/*   Updated: 2021/07/30 17:35:56 by sad-aude         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void            setGetPostCgiResponse(t_request &parsedRequest, const t_location *locationBlock, Config *serverConfigBlock)
{
...
void            doDelete(t_request &parsedRequest, Config *serverConfigBlock)
{
    if (checkPath(parsedRequest))
    {
...
std::string         getFullFinalResponse(t_request &parsedRequest)
{
    char    itoaTab[100];

	if (sprintf(itoaTab, "%lu", parsedRequest.fileContent.size()) > 0)
		return ("HTTP/1.1 " +  parsedRequest.statusCode + "\nContent-Type:" + parsedRequest.fileType + "\nContent-Length:" 
...
std::string         buildClientResponse(t_request &parsedRequest, const t_location *locationBlock, Config *serverConfigBlock)
{
	...
	if (parsedRequest.requestMethod != "DELETE" || parsedRequest.statusCode != "200 OK")
    {
		if (parsedRequest.statusCode == "301 Moved Permanently")
            ...
            setGetPostCgiResponse(parsedRequest, locationBlock, serverConfigBlock);
	}
    else
        doDelete(parsedRequest, serverConfigBlock);
	return (getFullFinalResponse(parsedRequest));
}
